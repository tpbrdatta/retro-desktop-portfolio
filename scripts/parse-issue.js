...
// Maps the issue form's human-readable wallpaper option to the CSS class
// suffix template.html expects (wp-teal, wp-hills, etc).
const WALLPAPER_KEYS = {
  'teal solid': 'teal',
  'hills': 'hills',
  'clouds': 'clouds',
  'tiled pattern': 'tiled',
  'blue abstract': 'abstract'
};

function resolveWallpaperKey(label) {
  const key = label.trim().toLowerCase();
  return WALLPAPER_KEYS[key] || 'teal';
}
...
function parseMarkdown(body) {
  const data = {
    name: '', title: '', wallpaper: 'teal', status: '🟢 Active',
  ...
  const matches = {
    ...
    wallpaper: body.match(/### Default Wallpaper\s+([\s\S]*?)(?=\n###|$)/),
  ...
  if (matches.wallpaper) {
    data.wallpaper = resolveWallpaperKey(matches.wallpaper[1].trim());
  }
...
function generateHTML() {
  ...
  html = html.replace(/\{\{NAME\}\}/g, values.name)
             ...
             .replace(/\{\{DEFAULT_WALLPAPER\}\}/g, values.wallpaper)
